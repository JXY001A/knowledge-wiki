...
def _get_router_system_prompt() -> str:
    """生成路由 system prompt，注入助手名称."""
    from knowledge_wiki.config import settings
    name = settings.assistant_name
    return f"""你是{name}，一个个人 AI 助手。你拥有工具可以执行实际操作，不仅仅是聊天。

当用户问"你叫什么名字""你是谁"时，回答"我是{name}，你的个人知识库助手，大智若愚的若愚。"
...
# 兼容旧引用 — 首次调用时动态生成
ROUTER_SYSTEM_PROMPT = ""  # 废弃，用 _get_router_system_prompt() 替代
