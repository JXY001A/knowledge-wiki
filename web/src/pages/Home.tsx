  ...
  return (
    <div>
      ...
      <section className="text-center pt-20 pb-12 px-4 max-w-2xl mx-auto">
        <span className="inline-block text-xs text-gray-400 tracking-wider uppercase mb-5">
          AI 自进化知识系统
        </span>
        ...
        <div className="flex gap-3 justify-center flex-wrap">
          <Link
            to="/chat"
            className="bg-accent text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-accent-dark transition-colors shadow-[0_2px_8px_rgba(77,107,254,.25)]"
          >
            开始对话
          </Link>
          <Link
            to="/admin"
            className="bg-white border border-gray-200 text-gray-700 px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors"
          >
            管理后台
          </Link>
          <Link
      ...
      <section className="max-w-4xl mx-auto px-4 pb-16">
        <h2 className="text-lg font-semibold text-center text-gray-900 mb-2">能做什么</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {features.map((f) => (
            <div
              key={f.label}
              className="bg-white border border-gray-100 rounded-lg p-5 hover:shadow-card transition-shadow duration-150"
            >
      ...
      <section className="max-w-xl mx-auto px-4 pb-20">
        <h2 className="text-lg font-semibold text-center text-gray-900 mb-8">五层架构</h2>
        <div className="flex flex-col gap-1.5">
          {layers.map((l) => (
            <div
              key={l.lvl}
              className={`${l.bg} rounded-lg px-4 py-2.5 flex items-center gap-3 text-sm text-gray-700`}
            >
              <span
                className={`w-7 h-7 rounded-full ${l.dot} text-white flex items-center justify-center text-xs font-semibold`}
              >
                {l.lvl}
              </span>
              {l.title}
