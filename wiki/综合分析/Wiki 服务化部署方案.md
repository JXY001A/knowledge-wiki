...
3. DevMechin 上 clone 仓库
4. 编写 wiki-mcp-server（ingest / query / lint / search）
5. 编写 wecom-bot-webhook（~150 行 Flask/FastAPI）
6. frpc.toml 新增 9300 + 9400 端口映射
7. 配置 systemd 服务（wiki-mcp + wecom-webhook，开机自启）
8. 注册企业微信，配置 Bot 回调 URL → `http://8.133.175.201:9400/webhook`
9. 端到端测试：Mac Claude Code → MCP → query + 手机微信 → Bot → ingest
